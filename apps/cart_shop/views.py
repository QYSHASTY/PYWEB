...
from .models import WishlistItemShop
...
def fill_wishlist_in_session(request):
    wishlist = request.session.get('wishlist', {})
    if request.user.is_authenticated and not wishlist:
        wishlist_items = WishlistItemShop.objects.filter(wishlist__user=request.user)
        for item in wishlist_items:
            wishlist[item.product.id] = item.quantity
...
# def fill_id_wishlist_in_session(request):
#     id_wishlist = request.session.get('id_wishlist', None)
#     if request.user.is_authenticated and not wishlist:
#         id_wishlist = Wishlist.objects.get(user=request.user).id
#         request.session['id_wishlist'] = id_cart
#     return id_wishlist
...
def save_product_in_wishlist(request, product_id):
    wishlist = WishlistItemShop.objects.filter(wishlist__user=request.user,
                                               product__id=product_id)
    if wishlist:
       ...
       product = get_object_or_404(Product, id=product_id)
       wishlist_user = get_object_or_404(Wishlist, user=request.user)
       wishlist_item = WishlistItemShop(wishlist=wishlist_user, product=product)
    wishlist_item.save()
...
class ViewWishAdd(View):
   def get(self, request, product_id):
       save_product_in_wishlist(request, product_id)
       return redirect('home:index')
